...
- **ファイル添付対応** — 画像・テキスト・メール等のファイルをチャットに添付可能。画像は LLM による視覚分析、テキストは内容の直接読み込み、その他ファイルは `read_document_file` ツールでテキスト抽出できます（10MB 上限）
- **DeskToDo MCP サーバー同梱** — タスク管理に特化した MCP サーバーを標準搭載。FTS5 全文検索・エンベディング意味検索・一括操作・変更履歴追跡など30以上のツールを提供します

## 📎 ファイル添付機能

チャット UI からファイルを添付して、AI に内容を読み取らせたり分析させたりできます。

### 添付方法

Chainlit のチャット入力欄にある 📎 クリップアイコンからファイルを選択して添付します。テキスト入力と同時に送信できます。

### 対応ファイル形式と処理方式

| ファイル種別 | 対応形式 | 処理方式 |
|---|---|---|
| **画像** | `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.bmp` | base64 エンコードして LLM に直接送信（画像認識・分析） |
| **テキスト** | `.txt`, `.md`, `.csv`, `.json`, `.py`, `.yaml`, `.yml`, `.xml`, `.html`, `.htm`, `.log`, `.ini`, `.cfg`, `.toml`, `.rst` | 内容をテキストとして読み込み、プロンプトに埋め込んで送信（UTF-8 / Shift-JIS 対応） |
| **その他** | 上記以外の全ファイル形式 | ファイルパス情報をテキストとして送信。`read_document_file` ツールで読み込み可能 |

### 制限事項・注意点

- **ファイルサイズ上限**: 10MB を超えるファイルは無視されます
- **テキストファイルの文字数上限**: 8,000 文字を超える場合は先頭から切り詰めて送信されます
- **セキュリティ**: 添付ファイルは一時ディレクトリに保存され、パストラバーサル攻撃を防ぐための検証が行われます。危険なパスパターン（`..`, `~` 等）を含むファイルは拒否されます
- **その他ファイルの処理**: 画像・テキスト以外のファイル（`.eml`, `.msg`, `.pdf` 等）は、AI にファイルパスを伝え、`read_document_file` ツールを使って DeskToDo MCP サーバー側でテキスト抽出する方式を推奨します
